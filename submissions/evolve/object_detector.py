...
class ObjectDetector:
    def __init__(self):
        ...
        lwfa_config = DetrConfig(
            num_channels=2, revision="no_timm"
        )  # DETR facebook/detr-resnet-50 architecture wit custom 2-channel input
        self.processor = DetrImageProcessor(lwfa_config)
        self.model = DetrForObjectDetection(lwfa_config)
        ...
        self.model.class_labels_classifier = nn.Linear(
            in_features=256, out_features=num_classes
        )
    ...
    def fit(self, X, y):
        ...
        for epoch in range(num_epochs):
            ...
            for images, targets in data_loader:
                ...
                # Feed the images to DETR
                inputs = self.processor(images=images, return_tensors="pt").to(
                    self.device
                )
                loss_dict = self.model(**inputs, labels=targets)
